    ...
    for(trans in transactions){
      ...
      if("sector" %in% trans_child_names){
        ...
        if(sector_vocab %in% c("1","2","DAC","DAC-3")){
          ...
          trans_sectors[sector_code]=value
        }
      }
    }
...
for(i in 1:length(unique.activities)){
  ...
  if(length(sec_vec)>0){
    sec_df_tmp = data.frame(
      sector_code=names(sec_vec),
      sector_percentage=sec_vec,
      aid = unique.activity
    )
    sec_df_tmp = data.table(sec_df_tmp)[,.(sector_percentage=sum(sector_percentage,na.rm=T)),by=.(sector_code,aid)]
...
sec_df = subset(sec_df,sector_code!="")
sec_df[,num_secs:=nrow(.SD),by=.(aid)]
sec_df$sector_percentage[which(sec_df$sector_percentage=="0")] = 1/sec_df$num_secs[which(sec_df$sector_percentage=="0")]

dat_sectors = merge(dat, sec_df, by="aid", allow.cartesian=TRUE)

dat_sectors$commitment = as.numeric(dat_sectors$commitment)*dat_sectors$sector_percentage
dat_sectors$commitment_cad = as.numeric(dat_sectors$commitment_cad)*dat_sectors$sector_percentage
dat_sectors$commitment_eur = as.numeric(dat_sectors$commitment_eur)*dat_sectors$sector_percentage
dat_sectors$commitment_gbp = as.numeric(dat_sectors$commitment_gbp)*dat_sectors$sector_percentage
dat_sectors$spend = as.numeric(dat_sectors$spend)*dat_sectors$sector_percentage
dat_sectors$spend_cad = as.numeric(dat_sectors$spend_cad)*dat_sectors$sector_percentage
dat_sectors$spend_eur = as.numeric(dat_sectors$spend_eur)*dat_sectors$sector_percentage
dat_sectors$spend_gbp = as.numeric(dat_sectors$spend_gbp)*dat_sectors$sector_percentage

save(dat_sectors,file="d_portal_sectors_micro.RData")
fwrite(dat_sectors,"d_portal_sectors_micro.csv")
