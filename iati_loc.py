...
class IatiFlat(object):
    def __init__(self):
        self.header = ["year","recipient_code","to_di_id","flow_code","category","finance_type","aid_type","usd_disbursement","short_description","purpose_code","sector_code","channel_code","long_description","ftc","pba","budget_or_transaction","budget_type","iati_identifier","incoming","location_xml"]
        self.dictionaries = {}
    ...
    def flatten_activities(self,root):
        ...
        for i in range(0,activity_len):
            ...
            if "location" in child_tags:
                ...
                location_xml = "<locations>"+"".join([str.strip(etree.tostring(elem)) for elem in locations])+"</locations>"
            else:
                location_xml = ""
            ...
            #If no activities or sectors, add a placeholder
            if len(activity_sectors.keys())==0:
                activity_sectors["99999"] = 1
            if len(activity_recipients.keys())==0:
                activity_recipients["99999"] = 1
                                ...
                                if use_activity_sectors:
                                    ...
                                    for activity_recipient_code in activity_recipients.keys():
                                        ...
                                        for activity_sector_code in activity_sectors.keys():
                                            ...
                                            row = [year,recip,to_di_id,flow_type_code,category,finance_type_code,aid_type_code,converted_value,short_description,pur_code,sec_code,channel_code,long_description,ftc,pba,b_or_t,budget_type,iati_identifier,incoming,location_xml]
                                            output.append(row)
                                else:
